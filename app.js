...
function showToast(message){
  ...
  window.__vermittlungToastTimer = setTimeout(function(){
...
function renderAuth(){if(currentUser){$('#loginView').classList.add('hidden');$('#appView').classList.remove('hidden');$('#userName').textContent=currentUser.email||'';navigate('dashboard');}else{$('#appView').classList.add('hidden');$('#loginView').classList.remove('hidden');}}
...
function renderCustomers(){const rows=customers.map(c=>`<tr><td>${esc((c.vorname||'')+' '+(c.nachname||''))}</td><td>${esc(c.firma||'')}</td><td>${esc(c.telefon||'')}</td><td>${esc(c.email||'')}</td><td>${esc((c.strasse||'')+' '+(c.plz||'')+' '+(c.ort||''))}</td></tr>`).join('');$('#main').innerHTML=page('Kunden',`<div class="table-wrap"><table class="table"><thead><tr><th>Name</th><th>Firma</th><th>Telefon</th><th>E-Mail</th><th>Adresse</th></tr></thead><tbody>${rows||'<tr><td colspan="5">Noch keine Kunden.</td></tr>'}</tbody></table></div>`)}
function renderCompanies(){const rows=companies.map(c=>`<tr><td>${esc(c.firmenname)}</td><td>${esc(c.ansprechpartner||'')}</td><td>${esc(c.gewerk||'')}</td><td>${esc(c.telefon||'')}</td><td>${esc(c.email||'')}</td></tr>`).join('');$('#main').innerHTML=page('Firmen / Subunternehmer',`<div class="table-wrap"><table class="table"><thead><tr><th>Firma</th><th>Ansprechpartner</th><th>Gewerk</th><th>Telefon</th><th>E-Mail</th></tr></thead><tbody>${rows||'<tr><td colspan="5">Noch keine Firmen.</td></tr>'}</tbody></table></div>`)}
...
async function showOrder(id){
  ...
  m.querySelector('#docForm').onsubmit=async e=>{
    ...
    if(error){alert(error.message);return}
    m.remove();showToast('Dokument verknüpft');showOrder(id);
  };
