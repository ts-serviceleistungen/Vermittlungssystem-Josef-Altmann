...
async function syncGoogle(){
  ...
  if(!u||!t){
    alert('Bitte zuerst URL und Sicherheitsschlüssel speichern.');
    navigate('google');
    return;
  }
  ...
  try{
    // Direkte Übertragung über ein unsichtbares HTML-Formular.
    // Dadurch wird die Browser-CORS-Sperre beim Apps-Script-Web-App-Endpunkt umgangen.
    ...
    if(s)s.textContent='Synchronisierung wurde an Google übergeben. Bitte kurz warten.';
    toast('Synchronisierung gestartet');
  }catch(e){
    alert('Google-Synchronisierung konnte nicht gestartet werden: '+e.message);
  }
}
