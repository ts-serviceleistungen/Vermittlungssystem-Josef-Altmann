...
function toast(message){const el=$('#toast');if(!el)return;el.textContent=message;el.classList.add('show');clearTimeout(window.__vermittlungToastTimer);window.__vermittlungToastTimer=setTimeout(()=>el.classList.remove('show'),2800);}
let currentUser=null, customers=[], companies=[], orders=[];
const CURRENT_PAGE_KEY='vermittlung_current_page';
...
function renderAuth(){if(currentUser){$('#loginView').classList.add('hidden');$('#appView').classList.remove('hidden');$('#userName').textContent=currentUser.email||'';const saved=localStorage.getItem(CURRENT_PAGE_KEY)||'dashboard';navigate(saved,false);}else{$('#appView').classList.add('hidden');$('#loginView').classList.remove('hidden');}}
...
function orderTable(list){if(!list.length)return '<p class="muted">Keine Aufträge vorhanden.</p>';return `<div class="table-wrap"><table class="table"><thead><tr><th>Nr.</th><th>Bereich</th><th>Status</th><th>Auftrag</th><th>Wert</th><th>Provision</th></tr></thead><tbody>${list.map(o=>`<tr><td><button class="secondary" onclick="showOrder('${o.id}')">${esc(o.auftragsnummer||'—')}</button></td><td>${esc(o.bereich)}</td><td><span class="badge status">${esc(o.status)}</span></td><td>${esc(o.beschreibung||'—')}</td><td>${money(o.auftragswert_netto)}</td><td>${money(o.provision)}</td></tr>`).join('')}</tbody></table></div>`}
...
function renderCustomers(){
  const rows=customers.map(c=>`<tr>
    <td>${esc(((c.vorname||'')+' '+(c.nachname||'')).trim()||'—')}</td>
    <td>${esc(c.firma||'')}</td><td>${esc(c.telefon||'')}</td><td>${esc(c.email||'')}</td>
    <td>${esc(((c.strasse||'')+' '+(c.plz||'')+' '+(c.ort||'')).trim())}</td>
    <td class="actions"><button class="secondary" onclick="editCustomer('${c.id}')">Bearbeiten</button><button class="close" onclick="deleteCustomer('${c.id}')">Löschen</button></td>
  </tr>`).join('');
  $('#main').innerHTML=page('Kunden',`
    <div class="toolbar"><button class="primary" onclick="newCustomer()">+ Neuer Kunde</button></div>
    <div class="table-wrap"><table class="table"><thead><tr><th>Name</th><th>Firma</th><th>Telefon</th><th>E-Mail</th><th>Adresse</th><th>Aktionen</th></tr></thead>
    <tbody>${rows||'<tr><td colspan="6">Noch keine Kunden.</td></tr>'}</tbody></table></div>`);
}
function customerForm(c={}){
  const title=c.id?'Kunde bearbeiten':'Neuen Kunden anlegen';
  const m=modal(`<h2>${title}</h2><form id="customerForm" class="form-grid">
    <label>Vorname<input name="vorname" value="${esc(c.vorname||'')}"></label>
    <label>Nachname<input name="nachname" value="${esc(c.nachname||'')}"></label>
    <label>Firma<input name="firma" value="${esc(c.firma||'')}"></label>
    <label>Telefon<input name="telefon" value="${esc(c.telefon||'')}"></label>
    <label>E-Mail<input name="email" type="email" value="${esc(c.email||'')}"></label>
    <label>Straße<input name="strasse" value="${esc(c.strasse||'')}"></label>
    <label>PLZ<input name="plz" value="${esc(c.plz||'')}"></label>
    <label>Ort<input name="ort" value="${esc(c.ort||'')}"></label>
    <label class="full">Notizen<textarea name="notizen">${esc(c.notizen||'')}</textarea></label>
    <div class="full actions"><button type="button" class="close" onclick="this.closest('.modal').remove()">Abbrechen</button><button class="primary">Speichern</button></div>
  </form>`);
  m.querySelector('#customerForm').onsubmit=async e=>{
    e.preventDefault(); const d=Object.fromEntries(new FormData(e.target).entries());
    let result=c.id?await db.from('vermittlung_kunden').update(d).eq('id',c.id):await db.from('vermittlung_kunden').insert(d);
    if(result.error){alert(result.error.message);return}
    m.remove(); navigate('customers');
  };
}
function newCustomer(){customerForm()}
function editCustomer(id){const c=customers.find(x=>x.id===id);if(c)customerForm(c)}
async function deleteCustomer(id){
  const c=customers.find(x=>x.id===id); if(!c)return;
  const name=((c.vorname||'')+' '+(c.nachname||'')).trim()||c.firma||'Kunde';
  if(!confirm(`Kunden „${name}“ wirklich löschen?`))return;
  const {error}=await db.from('vermittlung_kunden').delete().eq('id',id);
  if(error){alert('Kunde konnte nicht gelöscht werden. Falls der Kunde bereits einem Auftrag zugeordnet ist, muss er dort zuerst entfernt werden.\n\n'+error.message);return}
  navigate('customers');
}
function renderCompanies(){
  const rows=companies.map(c=>`<tr>
    <td>${esc(c.firmenname)}</td><td>${esc(c.ansprechpartner||'')}</td><td>${esc(c.gewerk||'')}</td><td>${esc(c.telefon||'')}</td><td>${esc(c.email||'')}</td>
    <td class="actions"><button class="secondary" onclick="editCompany('${c.id}')">Bearbeiten</button><button class="close" onclick="deleteCompany('${c.id}')">Löschen</button></td>
  </tr>`).join('');
  $('#main').innerHTML=page('Firmen / Subunternehmer',`
    <div class="toolbar"><button class="primary" onclick="newCompany()">+ Neue Firma / Subunternehmer</button></div>
    <div class="table-wrap"><table class="table"><thead><tr><th>Firma</th><th>Ansprechpartner</th><th>Gewerk</th><th>Telefon</th><th>E-Mail</th><th>Aktionen</th></tr></thead>
    <tbody>${rows||'<tr><td colspan="6">Noch keine Firmen / Subunternehmer.</td></tr>'}</tbody></table></div>`);
}
function companyForm(c={}){
  const title=c.id?'Firma / Subunternehmer bearbeiten':'Neue Firma / Subunternehmer anlegen';
  const m=modal(`<h2>${title}</h2><form id="companyForm" class="form-grid">
    <label>Firmenname<input name="firmenname" required value="${esc(c.firmenname||'')}"></label>
    <label>Ansprechpartner<input name="ansprechpartner" value="${esc(c.ansprechpartner||'')}"></label>
    <label>Telefon<input name="telefon" value="${esc(c.telefon||'')}"></label>
    <label>E-Mail<input name="email" type="email" value="${esc(c.email||'')}"></label>
    <label>Straße<input name="strasse" value="${esc(c.strasse||'')}"></label>
    <label>PLZ<input name="plz" value="${esc(c.plz||'')}"></label>
    <label>Ort<input name="ort" value="${esc(c.ort||'')}"></label>
    <label>Gewerk<input name="gewerk" value="${esc(c.gewerk||'')}"></label>
    <label class="full">Notizen<textarea name="notizen">${esc(c.notizen||'')}</textarea></label>
    <div class="full actions"><button type="button" class="close" onclick="this.closest('.modal').remove()">Abbrechen</button><button class="primary">Speichern</button></div>
  </form>`);
  m.querySelector('#companyForm').onsubmit=async e=>{
    e.preventDefault(); const d=Object.fromEntries(new FormData(e.target).entries());
    let result=c.id?await db.from('vermittlung_firmen').update(d).eq('id',c.id):await db.from('vermittlung_firmen').insert(d);
    if(result.error){alert(result.error.message);return}
    m.remove(); navigate('companies');
  };
}
function newCompany(){companyForm()}
function editCompany(id){const c=companies.find(x=>x.id===id);if(c)companyForm(c)}
async function deleteCompany(id){
  ...
  if(error){alert('Firma konnte nicht gelöscht werden. Falls sie bereits einem Auftrag zugeordnet ist, muss die Zuordnung zuerst entfernt werden.\n\n'+error.message);return}
  navigate('companies');
}
